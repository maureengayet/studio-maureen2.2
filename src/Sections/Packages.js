          ...
          <div className="packages-list overflow-hidden flex flex-col">
            {packages.map((pkg, index) => (
              <div key={index} className="package-item flex flex-col w-full">
                ...
                >
                  <h4 className="small-caps my-4 flex flex-row justify-between items-center">
                    {pkg.title} 
                    {isOpen(index) ? <MinusIcon className='size-5' /> : <PlusIcon className='size-5' />}
                  </h4>
                  <div
                    className={`transition-max-height duration-300 ease-in-out max-h-0 overflow-hidden ${isOpen(index) && 'max-h-96'}`}
                  >
