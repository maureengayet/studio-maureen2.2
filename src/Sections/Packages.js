            ...
            {packages.map((pkg, index) => (
              <div key={index} className="package-item flex flex-col w-full">
                ...
                >
                  ...
                  <div
                    className={`transition-max-height duration-300 ease-in-out max-h-0 overflow-hidden ${
                      isOpen(index) && "max-h-96"
                    }`}
                  >
                    ...
                    <ul className="my-4 mb-8">
                      {pkg.list.map((item, i) => (
                        <li key={i}>
                          <CheckIcon className="size-5 text-accent inline mr-2" />{" "}
                          {item}
                        </li>
                      ))}
                    </ul>
